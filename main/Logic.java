...
import plugin.FRCustom.Controller;
...
public class Logic extends Controller {
    ...
    public void Game_Start() {
        ...
        array.add(0, head);
        array.add(1, body1);
        Lime l = new Lime((int) (Math.random() * 1000 + 80), (int) (Math.random() * 600 + 60));
    ...
    */
    public void Game_Clock() {
        ...
        Rectangle c = null;
        Rectangle d = null;
        int corx = (int) (Math.random() * 1040 + 80);
        int cory = (int) (Math.random() * 640 + 60);
        ...
        if (head.getDirection() == 0) {
            if (a.intersects(b)) {
                score++;
                Lime l;
                for (Snake_part i : array) {

                    if (corx != i.getX() && cory != i.getY())
                        l = new Lime(corx, cory);
                    c = new Rectangle(corx, cory, 25, 25);
                    if (b.intersects(c)) {
                        corx = (int) (Math.random() * 1040 + 80);
                        cory = (int) (Math.random() * 640 + 60);
                    }
                }

                array.add(new Snake_body(array.get(array.size() - 1).getX(),
                        array.get(array.size() - 1).getY()));
        ...
        for (int i = 6; i < array.size(); i++) {
            d = new Rectangle(array.get(i).getX(), array.get(i).getY(), 25, 25);
            if (d.intersects(a)) {
                Game_Over();
            }
        }
    ...
    public void Game_Over() {
        ...
        array.add(head);
        array.add(body1);
        GUI.Game_Over(); /** DO NOT DELETE THIS */
    }
