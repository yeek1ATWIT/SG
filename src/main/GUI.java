...
import plugin.FRCustom.FRCanvas;
import plugin.FRCustom.FRCanvasFrame;
import plugin.FRCustom.FRShape2D;
...
public class GUI {
    ...
    private static int limes = logic.score;
    ...
    public static void LeaderBoard_Setup() {
        Object[] Hierarchy = new Object[100];
    ...
    public static void Game_GUI_Setup() {
    ...
    /**
     * Shows the Menu with all of the buttons in place.
     */
    ...
    /**
     * Shows the empty table of the LeaderBoard
     */
    ...
    /**
     * Shows the empty table of the LeaderBoard
     * and it shows the close button of the board as well.
     */
    ...
    private static void Show_Score() {
        Object[] Hierarchy = new Object[100];
        JLabel Label = new JLabel();
        Label.setBounds(430,240,300,300);
        Label.setText("Limes: "+limes+" | Time: "+Time_toString());
    ...
    /** 
     * Loads the placements of the LeaderBoard
    ...
    private static Object[] Load_LeaderBoard(Object[] Hierarchy, int index, int Limes, int Time, Color color) {
        ...
        Label.setText((index+1) + ". Limes: "+Limes+" | Time: "+Time_toString(Time));
        Label.setForeground(color); 
        Label.setFont(new Font("Verdana",3,18));
    ...
    /**
     * Converts the SW to the proper analog time.
     * @return
     */
    private static String Time_toString() {
    ...
    /**
     * Converts the given Time to the proper analog time.
     * @return
     */
    private static String Time_toString(int Time) {
        StringBuilder sb = new StringBuilder();
        if (Time >= (FPS*60*60)) {
            sb.append(String.valueOf(String.format("%.2f", (double) (Time / FPS / 3600 % 24)*1.0 / 100).toCharArray(), 2, 2));
            sb.append(":");
        }
        if (Time >= (FPS*60)) {
            sb.append(String.valueOf(String.format("%.2f", (double) (Time / FPS / 60 % 60)*1.0 / 100).toCharArray(), 2, 2));
            sb.append(":");
        }
        sb.append(String.valueOf(String.format("%.2f", (double) (Time / FPS % 60)*1.0 / 100).toCharArray(), 2, 2));
        sb.append(":");
        sb.append(String.valueOf(String.format("%.2f", (double) (Time % FPS)*1.0 / FPS).toCharArray(), 2, 2));
    ...
    /**
     * Responsible for keeping track of the Stop-watch GUI
     * and makes sure that the GUI fits into the window
     * properly.
     */
    private static void SW_format() {
        if (Game_switch) {
            SW += 1; 
            ((JLabel) Game_GUI2.shapes[2]).setText(String.valueOf(limes));
            ((JLabel) Game_GUI2.shapes[3]).setText(Time_toString());
            if (SW == (FPS*60)) {
    ...
    /**
     * Responsible for refreshing the components many times per second.
     */
